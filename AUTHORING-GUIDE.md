...
└── assets/         ← images and downloadable flyers
```
...
**Step 1.** Drop the file into `assets/`. Use a clear filename, no spaces: `aepax-civil-flyer.pdf`, not `Civil Flyer Final v2.pdf`.

**Step 2.** Copy an existing `<article class="flyer">` block in the Resources section and edit:

```html
<article class="flyer">
  <div class="flyer-preview">
    <span class="flyer-spec">DOC.04 · PDF · A4</span>
    <img src="assets/aepax-civil-preview.jpg" alt="Civil engineering capability brief preview" loading="lazy" />
  </div>
  <div class="flyer-body">
    ...
    <div class="flyer-downloads">
      <a class="flyer-dl" href="assets/aepax-civil-flyer.pdf" download="Aepax-Civil-Brief.pdf">
        <!-- download icon SVG -->
        Download PDF
      </a>
    </div>
...
Key things:
- `download="Aepax-Civil-Brief.pdf"` forces the file to download with a clean filename, regardless of how it's stored.
- The preview image (`.flyer-preview img`) needs to be a separate image. For a PDF, export the first page as a JPG or PNG and use that.
- `loading="lazy"` means the browser only loads the image when needed. Always include it.

### Add a contact channel

Find `<div class="contact-channels">`. Copy a `<a class="contact-row">` and edit. Use the right URL scheme:
...
- **A real contact form.** The current contact section uses `mailto:` links. A proper form needs a backend. Easiest: [Formspree](https://formspree.io/), [Netlify Forms](https://www.netlify.com/products/forms/), or [Web3Forms](https://web3forms.com/). All take 5 minutes to wire up.
